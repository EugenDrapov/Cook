...
begin
  with DM, DM.DSConfPrn.DataSet do
    begin
      ...
      frxReport1.Variables['vdocid'] := id;
  ...
  with DM, DM.DSConfPrn.DataSet do
    begin
      ...
      frxReport1.LoadFromFile(ExtractFilePath(Application.ExeName)+'Templates\'
                +FieldByName('REPORTFILE').AsString);
      id:=CurrentDataSource.DataSet.FieldByName('id').AsInteger;
      db:=QuotedStr(DateToStr(DM.BeginPeriod));
      de:=QuotedStr(DateToStr(DM.EndPeriod));
      frxReport1.EngineOptions.DestroyForms:=false;
      frxReport1.Variables['dep_id'] := DSDepartments.DataSet.FieldByName('id').AsInteger;
      frxReport1.Variables['vdocid'] := id;
      frxReport1.Variables['beginperiod'] := db;
      frxReport1.Variables['endperiod'] := de;
  ...
  for i:=0 to MainDBGrid.Columns.Count-1 do
    begin
      ...
      if (UpperCase(MainDBGrid.Columns[i].FieldName)='SUMMA')
        ...
        MainDBGrid.Columns[i].Footer.DisplayFormat:='';
